...
pub(crate) fn convert_unified_addresses(
    ...
    for metadata in &unified_accounts.address_metadata {
        ...
        let ua_str = {
            ...
            // A present receiver type is required in the reconstructed address;
            // an absent one is omitted. This mirrors the `true`/`false` semantics
            // of the previous `UnifiedAddressRequest::new` API.
